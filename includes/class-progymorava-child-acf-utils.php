...
class Progymorava_Child_Acf_Utils {
	...
	private static function uses_placeholder( $field ) {
		...
		return in_array(
			...
			true
		) || 1 === preg_match( '/^(home_training_card_\d+|about_team_member|services_(coach|physio)_\d+|services_nutrition|services_prices_cta)_image$/', $field_name );
	}
